...
elseif (CModule::IncludeModule("vote"))
{
	...
	foreach ($arResult["QUESTIONS"] as $qID => $arQuestion){
		if(in_array($qID, $arParams["QUESTIONS_ID"])){
			$arReport[$qID]["QUESTION"] = $arQuestion["QUESTION"];
			$arReport[$qID]["ANSWERS"] = array();
			foreach($arQuestion["ANSWERS"] as $aID => $arAnswer){
				$arReport[$qID]["ANSWERS"][] = $arAnswer["MESSAGE"];
			}
		}
	}
	
	// https://cbre.rentnow.ru/bitrix/admin/vote_user_votes_table.php?lang=ru&VOTE_ID=79 
