...
elseif (intval($arParams["VOTE_ID"]) <= 0):
	if(empty($arParams["VOTE_ID"])) {
		ShowError(GetMessage("VOTE_EMPTY"));
		return;
	}
	if (CModule::IncludeModule("vote")) {
		//$test = CVote::GetList($by="s_id", $order="desc", array("URL" => $arParams["VOTE_ID"]), $is_filtered = false);
		//$test = CVote::GetListEx(array(), array("URL" => $arParams["VOTE_ID"]));
		$sqlGetVoteId = "SELECT id FROM b_vote WHERE URL='".$arParams["VOTE_ID"]."'";
		$err_mess = (CForm::err_mess())."<br>Function: GetByID<br>Line: ";
		$resVote = $DB->Query($sqlGetVoteId, false, $err_mess.__LINE__);

		if($itemVote = $resVote -> GetNext()) {
			$arParams["VOTE_ID"] = $itemVote['id'];
		} else {
			ShowError(GetMessage("VOTE_EMPTY"));
			return;
		}
